            ...
            if (D.initialized && 0 !== D.slides.length) {
                ...
                if (D.activeIndex === D.slides.length && (D.activeIndex = D.slides.length - 1), D.activeIndex < 0 && (D.activeIndex = 0), D.slides[D.activeIndex]) {
                    if (D.calcVisibleSlides(a), D.support.classList) {
                        ...
                        ///////////BY GA////////////////
                        /*jQuery("#slider_divs").css({
                            "padding-left": "729.2px",
                            "padding-right": "729.2px"
                        });*/
                        var numbers=0;

                        jQuery(".swiper-slide").each(function(){
                            jQuery(this).removeClass('small-Visible');
                            numbers++;
                        });

                        var two_before = Number(jQuery('.swiper-slide-active').attr('sl_id')) + Number( -2);
                        var two_after = Number(jQuery('.swiper-slide-active').attr('sl_id')) + Number( 2);

                        var two_beforex = Number(jQuery('.swiper-slide-active').attr('sl_id')) + Number( -3);
                        var two_afterx = Number(jQuery('.swiper-slide-active').attr('sl_id')) + Number( 3);

                        var tb_ad_class = '.sl-'+two_before;
                        var ta_ad_class = '.sl-'+two_after;
                        var tb_ad_classx = '.sl-'+two_beforex;
                        var ta_ad_classx = '.sl-'+two_afterx;
                        jQuery(tb_ad_classx).removeClass('swiper-slide-visible');
                        jQuery(ta_ad_classx).removeClass('swiper-slide-visible');
                        var tb_re_class = '.sl-'+(Number(two_before)+Number(1));

                        if(two_before >0){
                            jQuery(tb_re_class).removeClass('small-Visible');
                            jQuery(tb_ad_class).addClass('small-Visible');
                        }

                        if(two_after >0){
                            jQuery(ta_ad_class).addClass('small-Visible');
                        }

                        //jQuery('.small-Visible').css("margin-left","-5px;");
                        jQuery('.details_div').hide();
                        jQuery('.swiper-slide-active div .details_div').show();
                        //console.log('sl_id#'+(jQuery('.swiper-slide-active').attr('sl_id'))+' div .details_div');
                        //console.log(two_before+" | "+ two_after+"----------"+numbers);
