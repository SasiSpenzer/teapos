...
if(!empty($Mainarray)){
    foreach($Mainarray as $each_date){

        if($each_date['Date'] == date("Y-m-d")){

            $today_start_cash = isset($each_date ['startCash']) ? trim($each_date ['startCash']) : 00.00;
            $today_cash_sales = isset($each_date ['CashSales']) ? trim($each_date ['CashSales']) : 00.00;
            $today_card_sales = isset($each_date ['CardSales']) ? trim($each_date ['CardSales']) : 00.00;
...
if(isset($_POST['done'])){
    ...
    if (!empty($_POST['total_display_final']) || $_POST['total_display_final'] == '00' ) {
        ...
        if (!empty($_POST)) {
            ...
            $cash_feed_array['end_or_start'] = 0;
            ...
            $cash_feed_array['plus_or_minus'] = isset($_POST ['hidden_def']) ? trim($_POST ['hidden_def']) : '';;
            ...
            try {
                ...
                $_SESSION['start_cash_session'] = date('Y-m-d');
                        ...
                        header('Location:LogOutReports.php');
                        exit;
...
//        });
    ...
    $('#add_by_section').click(function(){
        ...
        var alertdis = $("#total_display_final").val();
        var endtt = $("#box_end_total").val();


        var num = endtt-alertdis ;
        var numfix = num.toFixed(2);
        $("#box_def").html(numfix);
        $('input[name=hidden_def]').val(numfix);
...
<div class="wrapper">
    ...
    <table border="1" width="" class=" ">
        <tr>
            <th>Start Cash</th>
            <th>Cash Sales</th>
            <th>Card Sales</th>
            <th>Total End Cash</th>
            <th>+/-</th>
        </tr>
        <tr>
            <td id=""><?php if($today_start_cash) echo $today_start_cash; ?> <input value="<?php if($today_start_cash) echo $today_start_cash; ?>" id="box_start_cash" type="hidden"/></td>
            <td id=""><?php if($today_cash_sales) echo $today_cash_sales; ?> <input value="<?php if($today_cash_sales) echo $today_cash_sales; ?>" id="box_cash_cash" type="hidden"/></td>
            <td id=""><?php if($today_card_sales) echo $today_card_sales; ?>  <input value="<?php if($today_card_sales) echo $today_card_sales; ?>" id="box_card_cash" type="hidden"/></td>
            <td id="box_total"> <?php echo number_format($today_card_sales + $today_cash_sales + $today_start_cash,2)  ; ?><input value="<?php echo $today_card_sales + $today_cash_sales + $today_start_cash ; ?>" id="box_end_total" type="hidden"/></td>
            <td id="box_def">  </td>
    ...
    <div class="calculator-wrap">
        <div class="keypad-row">
            <input value="" id="jjj" name="hidden_def" value="bar" type="hidden"/>
...
<div align="center">
    ...
    font-size: 16px;
    margin: 4px 2px;
    cursor: pointer;" type="submit" value="Go for Reports" name="done" class="btn btn-primary"/>
